...
setup_bucket=false
setup_vm=false
setup_strawman=false
benchmark=false

REPO="gcsfuse-strawman"

for cmd in "$@"; do
    case $cmd in
        setup_bucket) setup_bucket=true ;;
        setup_vm) setup_vm=true ;;
        setup_strawman) setup_strawman=true ;;
...
if [[ "$setup_strawman" == "true" ]]; then
    ...
    if [[ ! -d ${REPO} ]]; then
        ...
        cd ${REPO}
        git pull origin akeso-strawman
        git checkout -b akeso-strawman
        cd -
