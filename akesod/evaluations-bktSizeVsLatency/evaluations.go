...
func main() {
	...
	if len(os.Args) > 1 && os.Args[1] == "prepare" {
		ensureBucketsExist(ctx, bucketSizes, strategies)
		GenerateBucketFillingFiles(bucketSizes, strategies)
		setSoftDeleteOff(bucketSizes, strategies)
		setNotifications(bucketSizes)
	} else {
		fmt.Println("Assuming Buckets and Environment is prepared for benchmarking. Run `./evaluation prepare` first if not.")
		CopyFilesToBucket(bucketSizes, strategies)
		_ = BenchmarkReencryptions(bucketSizes, strategies)
	}
}

func setNotifications(bucketSizes map[string]int) {
	var wg sync.WaitGroup
	for sizeKey := range bucketSizes {
		wg.Add(1)
...
func setSoftDeleteOff(bucketSizes map[string]int, strategies []string) {
	var wg sync.WaitGroup
	for sizeKey := range bucketSizes {
...
func ensureBucketsExist(ctx context.Context, bucketSizes map[string]int, strategies []string) {
	var wg sync.WaitGroup
	for sizeKey := range bucketSizes {
		for _, strategy := range strategies {
			wg.Add(1)
...
func CopyFilesToBucket(bucketSizes map[string]int, strategies []string) {
	uploadLimiter := rate.NewLimiter(rate.Every(200*time.Millisecond), 1)

	for sizeKey, sizeInKB := range bucketSizes {
		numFiles := sizeInKB / fileSizeKB
		if numFiles == 0 && sizeInKB > 0 && fileSizeKB > 0 {
			numFiles = 1
		}
		...
		for i := 1; i <= numFiles; i++ {
			fileName := fmt.Sprintf("%dKB-%d.txt", fileSizeKB, i)
...
func BenchmarkReencryptions(bucketSizes map[string]int, strategies []string) string {
	header := "#BucketSize"
	for _, strategy := range strategies {
		header += "\t" + strategy
	}
	...
	for sizeKey, sizeInKB := range bucketSizes {
		...
		fmt.Printf("\n=== Starting benchmark for size %s (%d of %d) ===\n", sizeKey, sizeIndex, totalSizes)
		...
		for _, strategy := range strategies {
			...
			var akesoOperationStartTime time.Time
			if strategy == "akeso" {
				// Start with a clean time window for this size
				effectiveStartOfOperations := time.Now()
				akesoQueryStartTimeForFilter = effectiveStartOfOperations.Add(-10 * time.Second)
			...
			for i := 1; i <= numFiles; i++ {
				sizeBasedFileName := fmt.Sprintf("%dKB-%d.txt", fileSizeKB, i)
				var cmd *exec.Cmd
				...
				case "cmek-hsm":
					...
					cmd = exec.Command(cmdParts[0], cmdParts[1:]...)
				case "akeso":
					cmd = exec.Command("../cloud-cp", "-strategy", "akeso", "-maxReenc", "50", "-key", uploadKey, "-updateKey", updateKey, "-dekOverride", "dek.bin", gcsObjectURL)
					if i == 1 {
						akesoOperationStartTime = time.Now()
						// Update the query filter to be more precise
						akesoQueryStartTimeForFilter = akesoOperationStartTime.Add(-5 * time.Second)
						fmt.Printf("Akeso: Updated log query filter to: %s\n", akesoQueryStartTimeForFilter.Format(time.RFC3339))
					}
				default:
					cmd = exec.Command("../cloud-cp", "-strategy", strategy, "-key", uploadKey, "-updateKey", updateKey, gcsObjectURL)
				}
			...
			if strategy != "akeso" {
				...
				// Add a small delay to ensure logs are available
				time.Sleep(sizeSeparationDelay)
			...
			logQueryLoop:
				for logRetryCount < maxLogRetries {
					...
					for {
						...
						if strings.Contains(payloadStr, "[UPD]") {
							...
							// Ensure filename from log matches expected pattern for current bucket
							expectedFilePrefix := fmt.Sprintf("%dKB-", fileSizeKB)
							if !strings.HasPrefix(fileNameFromLog, expectedFilePrefix) {
								continue
							}

							// Additional validation: check if the log timestamp is reasonable
							logTimestamp := entry.Timestamp
							if logTimestamp.Before(akesoOperationStartTime.Add(-30 * time.Second)) {
								fmt.Printf("Akeso: Skipping old log entry from %s for file %s\n", logTimestamp.Format(time.RFC3339), fileNameFromLog)
								continue
							}
							...
							if !logFileTracker[fileNameFromLog] {
								if akesoFoundFiles < numFiles {
									...
									fmt.Printf("Akeso: Found new log for file: %s (timestamp: %s)\n", fileNameFromLog, logTimestamp.Format(time.RFC3339))
				...
				if akesoFoundFiles < numFiles {
					fmt.Printf("Akeso: Process completed for size %s, but only found logs for %d out of %d expected files.\n", sizeKey, akesoFoundFiles, numFiles)
				} else {
				...
				if akesoFoundFiles > 0 {
					...
					if okMin && okMax {
						if maxActualEnd > minActualStart {
							...
							durationSecondsJustCloud := float64(durationNs) / 1e9
							durationSecCloudAkeso := float64(maxActualEnd-int64(akesoOperationStartTime.UnixNano())) / 1e9

							fmt.Printf("Akeso strategy for bucket %s:\n", sizeKey)
							...
							fmt.Printf("  Duration of Cloud Functions (s): %.6f\n", durationSecondsJustCloud)
							fmt.Printf("  Duration of Cloud Function + Akesod runtime: %.6f\n", durationSecCloudAkeso)
							sizeReencTimes[strategy] = durationSecCloudAkeso
						} else {
			...
			fmt.Printf("Processed strategy %s for bucket size %s.\n", strategy, sizeKey)
		}
		...
		fmt.Printf("Results for bucket size %s written to %s\n", sizeKey, filepath.Base(datFile))
		fmt.Printf("=== Completed benchmark for size %s ===\n", sizeKey)

		// Add delay between different sizes
		if sizeIndex < totalSizes {
			fmt.Printf("\nWaiting %v before starting next size to ensure log separation...\n", sizeSeparationDelay)
			time.Sleep(sizeSeparationDelay)
		}
	}
...
func GenerateBucketFillingFiles(bucketSizes map[string]int, strategies []string) {
	var wg sync.WaitGroup
	...
	for sizeKey, sizeInKB := range bucketSizes {
		...
		fmt.Printf("For Bucket Size %s (%dKB): files required are: %d\n", sizeKey, sizeInKB, numFiles)

		for num := 1; num <= numFiles; num++ {
			wg.Add(1)
			go func(sKB, n int) {
				defer wg.Done()
				if err := createFile(fileSizeKB, n, outputDir); err != nil {
